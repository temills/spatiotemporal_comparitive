...
preprocess_model <- function(df_model) {
  ...
  if ("sample" %in% colnames(df_model)) {
    df_model <- df_model %>% filter(sample%%10000==0) %>%
...
write_preprocessed_data <- function() {
  ...
  model_paths = c(lot_path, lot_no_recursion_path, gpnc_path, gpsl_path, ridge_path, true_path, prev_path, lin_path)
  models = c("lot", "lot_no_recursion", "gpnc", "gpsl", "ridge", "true", "prev", "lin")
...
read_model_fit_data <- function(participant) {
  path <- "model_fits/LLs/"
  models = c("lot", "lot_no_recursion", "gpnc", "gpsl", "ridge", "lin")#,# "ss")#, "prev")#, "true")
  df <- data.frame()
  for (i in seq_along(models)) {
    model_name <- models[i]
