...
def run_test():
    ...
    for participant_name in ["kid_chs","monkey",  "adult"]:
        for model_name in ["lot", "gpnc", "gpsl", "ridge", "lin", "lin_prev", "transformer_2", "transformer_3", "transformer_13", "lot_no_recursion"]:
    ...
    for (participant_name, model_name) in pairs:
        ...
        print(model_name)
        df_participant_model = pd.read_csv("preprocessed_data/" + model_name + "_" + participant_name + ".csv", dtype={"func.name": "string"})
        out_path = "model_fits/test/" + model_name + "_" + participant_name + ".csv"
