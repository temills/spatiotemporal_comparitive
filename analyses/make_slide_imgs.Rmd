...
preprocessed_data_dir <- "../paper_repo/preprocessed_data/"
fig_dir <- "../../slide_figs/"
```

```{r}
passed <- (read.csv("../human_experiments/dots_app_kids/data/clean_data/data.csv") %>%
            group_by(subject_id) %>%
             mutate(max_session = max(session)) %>%
             filter(max_session > 0))$subject_id
```

```{r}
ages <- read.csv("../human_experiments/dots_app_kids/data/clean_data/data.csv") %>%
        distinct(subject_id, exact_age) %>%
        rename(child__hashed_id=subject_id)

```


```{r}
demo <- read.csv("../human_experiments/dots_app_kids/data/demographics.csv")
# tmp <- (demo %>%
#        filter(response__video_privacy %in% c("public", "scientific")) %>% 
#        filter(child__hashed_id %in% passed) %>%
#       arrange(child__hashed_id))$child__hashed_id
# 
# print(tmp)
```
```{r}
tmp <- (demo %>%
       filter(response__video_privacy %in% c("public", "scientific")) %>% 
       filter(child__hashed_id %in% passed) %>%
       merge(ages, by="child__hashed_id") %>%
      arrange(child__hashed_id))  %>%
       distinct(child__hashed_id, exact_age)
write.csv(tmp, "sam.csv")
```

```{r}
tmp <- read.csv("../human_experiments/dots_app_kids/data/clean_data/data.csv") %>%
      filter(subject_id=="dUE7P6")
```

# Model example predictions with all models
```{r, fig.height=12,fig.width=9.5}
make_extended_prediction_plot <- function(colormap, pol_part) {
  ...
  df_pol <- preprocess_model_extended(read.csv(paste0(data_dir, 'models/extended_predictions/ridge_extended.csv'))) %>% mutate(model="Polynomial") %>%
            filter(curr_tpt==15) %>%
            
  
   df_gpsl <- preprocess_model_extended(read.csv(paste0(data_dir, 'models/extended_predictions/comp_gp_extended.csv'))) %>%
     mutate(model="Comp. GP") %>%
     mutate(func=paste0(func_x, "\n", func_y))
  ...
  df_model <- bind_rows(bind_rows(bind_rows(bind_rows(df_lot, df_gpnc), df_lin), df_pol), df_gpsl) %>%
              mutate(hyp=func) %>%
              merge(bounds_df, by="func.name") %>%
              mutate(scale_by = (max_x-min_x)) %>%
  ...
  get_df_path <- function(seq, t, selected_particles) {
      df_model_path <- df_model %>%
                ...
                mutate(selected_particle = ifelse(model=="GP", selected_particles[3],
                                                  ifelse(model=="Polynomial", selected_particles[2],
                                                         ifelse(model=="Linear", selected_particles[1],
                                                                ifelse(model=="Comp. GP", selected_particles[4],
                                                                        selected_particles[5]))))) %>%
                                                  ...
                                                  ifelse(model=="Polynomial", selected_particles[2],
                                                         ifelse(model=="Linear", selected_particles[1],
                                                                ifelse(model=="Comp. GP", selected_particles[4],
                                                                      selected_particles[5]))))) %>%
                filter(r==selected_particle) %>%
                mutate(model = factor(model, levels=c("Linear", "Polynomial", "GP", "Comp. GP", "LoT"))) %>%
                filter(func.name==seq) %>% 
                filter(curr_tpt == t) %>%
  ...
  plot_extended_predictions <- function(df_model_path, df_model_points, df_true) {
      x_min = min(min(df_true$true_x), min(df_model_points$pred_x)) + 0.2
      ...
      p <- ggplot() +
          ...
          scale_color_manual(values = colormap,
                             limits = c("LoT", "GP", "Linear", "Polynomial", "Comp. GP"),
                             labels = c( "LoT" = "LoT", "GP" = "GP", "Linear" = "Linear", "Polynomial"="Polynomial", "Comp. GP" = "Comp. GP")) +
          ...
          guides(alpha="none",
                 color = "none",
                 shape = "none") +
          facet_grid(rows = vars(func.name),
                     cols = vars(model),
                     labeller = labeller(model = c("Polynomial" = "Parametric"))) +
          coord_fixed(ratio=1,
                      xlim=c(x_min, 0.8),
                      ylim=c(-0.45, 0.1))
      print(y_min)
  ...
  arg_list <- list(
                   list("zigzag_1", 5, list(3,2,3,7,1)),
                   list("plus", 6, list(9,5,2,1,3)),
                   list("stairs_2", 8, list(12,4,15,18,1)) #1, 4
                   )
  ...
  model_levels <- c("Linear", "Polynomial", "GP", "Comp. GP", "LoT")
...
#for (i in (1:20)) {
p <- make_extended_prediction_plot(colormap, i)
#}
ggsave(plot=p, paste0(fig_dir, "model_extended.png"), bg="white", width=11, height=8.5)
```
...
df_stimuli <- read.csv(paste0(data_dir, 'stimuli.csv')) %>% rename(func.name=seq_id)
df_model <- preprocess_model_extended(read.csv(paste0(data_dir, 'models/extended_predictions/ridge_extended.csv'))) %>%
            #filter(r==p) %>%
           # mutate(pred_x = pred_x_from_true,
           #        pred_y = pred_y_from_true) %>%
          filter(r<6) %>%
          filter(curr_tpt==15) %>%
          mutate(curr_tpt=t) %>%
          mutate(model="GP")
...
ggplot() +
        ...
        geom_path(data=df_model_path, aes(x=pred_x, y=-pred_y, group=model, color=model), 
                   linetype="solid", alpha=0.6, linewidth=0.8) +
        #geom_path(data=df_model_path, aes(x=pred_x_from_true, y=-pred_y_from_true, group=model), color="purple", 
        #           linetype="solid", alpha=0.6, linewidth=0.8) +
        geom_point(data=df_model_points %>% filter(prediction_tpt<=curr_tpt),
                   aes(x=pred_x, y=-pred_y, group=model, color=model),
                   size=1, alpha=0.9) +
...
```{r}
ggplot(data=df_lot) +
  geom_bar(aes(x=true_x, fill="Parametric")) +
scale_fill_manual(values = c("Parametric" = "#62C1C6"), 
                    name = "") +
  my
```
...
final_plot
ggsave(plot=final_plot, paste0(fig_dir, "example_neurips_predictions_lot_only.png"), width=10.4, height=5.6, bg="white")
```
...
funcs <- c("triangle_spiral", "alternating_diff_1", "increasing_lines", "growing_stairs_2")
widths <- c(2.7, 3.6, 3.4, 2.7)

for (func_idx in (1:length(funcs))) {
  func <- funcs[func_idx]
  w <- widths[func_idx]
  plots <- make_sequence_plots(func, t_min=2, t_max=20)
  print(plots[[1]])
  dir.create(paste0(fig_dir,  "/", func))
  for (i in (1:(length(plots)))) {
    print(i)
    ggsave(plot=plots[[i]], paste0(fig_dir,  "/", func, "/", func, "_", i, ".png"), width=w, height=2, bg="transparent")
  }
}
...
# 
# func <- "alternating_diff_1"
# plots <- make_sequence_plots(func, t_min=1, t_max=20)
# plots <- lapply(plots, function(p) {
#   p + theme(plot.margin = margin(0.05, 0.01, 0.05, 0.01, "in"))
# })
# p2 <- grid.arrange(grobs=plots, nrow=1)
...
# Make videos comparing single participant with single model
```{r, echo=F, message=F}
# Preprocessing
logsumexp <- function(x) {
  max_x <- max(x)
...
df_tpt_0 <- df_adult_vids %>%
            filter(tpt==0) %>%
            distinct(true_x, true_y, tpt, func.name)
...
#"LoT", "Nonrecursive LoT", "GP", "Comp. GP", 
model_list <- c("Comp. GP")#"Polynomial", "Linear", "Linear + Prev.", "Transformer (2)", "Transformer (3)", "Transformer (13)")
...
model_dfs <- list("LoT" = preprocess_df_model(read.csv(paste0(data_dir, "models/lot.csv"))),
                  ...
                  "Polynomial" = preprocess_df_model(read.csv(paste0(data_dir, "models/ridge.csv"))),
...
```{r}
frame_path <- "../../videos/frames/"
make_func_vid <- function(funcname, df_participant, df_model) {
  # filter once
  df_seq <- df_participant[df_participant$func.name == funcname, ]
  df_seq <- df_seq[order(df_seq$tpt), ]

  df_model_agg <- df_model %>%
    filter(func.name == funcname) %>%
    group_by(tpt, pred_x, pred_y) %>%
    summarize(posterior = sum(posterior), .groups="drop")

  # ranges
  x_min <- min(df_seq$true_x, df_seq$pred_x, na.rm=TRUE)
  x_max <- max(df_seq$true_x, df_seq$pred_x, na.rm=TRUE)
  y_min <- min(df_seq$true_y, df_seq$pred_y, na.rm=TRUE)
  y_max <- max(df_seq$true_y, df_seq$pred_y, na.rm=TRUE)

  range_x <- (x_max-x_min)
  range_y <- (y_max-y_min)
  shift_x <- range_x*1.5

  out_dir <- file.path(out_folder, funcname)
  dir.create(out_dir, showWarnings=FALSE, recursive=TRUE)

  png(
    filename = file.path(out_dir, "%04d.png"),
    width = 2000, height = 1000, res = 300, bg="white"
  )

  for (i in (min(df_seq$tpt)+1):max(df_seq$tpt)) {
    df_tpt <- df_seq[df_seq$tpt == i, ]
    df_hist <- df_seq[df_seq$tpt < i, ]

    # base plot
    p1 <- ggplot() +
      geom_path(data=df_hist, aes(x=true_x, y=true_y),
                linewidth=0.25, linetype="dotted") +
      geom_point(data=df_hist[df_hist$tpt==max(df_hist$tpt),],
                 aes(x=true_x, y=true_y, color="agent1"), size=1) +
      geom_point(data=df_hist[df_hist$tpt==max(df_hist$tpt),],
                 aes(x=true_x, y=true_y, color="agent2"), size=1) +
      geom_point(data=df_hist, aes(x=true_x, y=true_y), size=3) +
      # shifted
      geom_path(data=df_hist, aes(x=true_x+shift_x, y=true_y),
                linewidth=0.25, linetype="dotted") +
        geom_point(data=df_hist[df_hist$tpt==max(df_hist$tpt),],
                 aes(x=true_x+shift_x, y=true_y, color="agent1"), size=1) +
      geom_point(data=df_hist[df_hist$tpt==max(df_hist$tpt),],
                 aes(x=true_x+shift_x, y=true_y, color="agent2"), size=1) +
      geom_point(data=df_hist, aes(x=true_x+shift_x, y=true_y), size=3) +
      scale_color_manual(
        labels = c("agent1"=agent1, "agent2"=agent2),
        values=c("agent1"="maroon", "agent2"="#516797"),
        breaks = c("agent1","agent2")
      ) +
      coord_equal(
        xlim=c(x_min-range_x*.2, x_min+(range_x*2.7)),
        ylim=c(y_min-(range_y*.2), y_min+(range_y*1.2))
      ) +
      theme_void() +
      theme(
        panel.background = element_rect(fill="white"),
        legend.text=element_text(size=14, margin = margin(r=20, l=10)),
        legend.title=element_blank(),
    ...
    # add next true point
    p2 <- p1 +
      geom_point(data=df_tpt, aes(x=true_x, y=true_y),
                 color="gold", shape=8, size=3, stroke=2.0) +
      geom_point(data=df_tpt, aes(x=true_x+shift_x, y=true_y),
                 color="gold", shape=8, size=3, stroke=2.0)
    ...
    if (i < 3) {
      ...
      df_model_tpt <- df_model_agg[df_model_agg$tpt == i, ]
      p3 <- p2 +
        geom_point(data=df_model_tpt,
                   aes(x=pred_x+shift_x, y=pred_y,
                       alpha=log(posterior), color="agent2"),
                   size=2, stroke=1) +
        geom_point(data=df_tpt,
                   aes(x=pred_x, y=pred_y, color="agent1"),
...
for (i in (1:length(model_list))) {
  agent2 <- model_list[i]
  print(agent2)
  df_model <- model_dfs[[agent2]] %>% mutate(pred_y=-pred_y, true_y=-true_y)
  for (j in (1:length(participant_list))) {
    agent1 <- participant_list[j]
    print(agent1)
    df_participant <- participant_dfs[[agent1]] %>% mutate(pred_y=-pred_y, true_y=-true_y)
    out_folder <- paste0(frame_path, gsub(" ", "_", agent1), "_", gsub(" ", "_", agent2), "/")
