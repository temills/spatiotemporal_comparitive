...
ggplot(data=df %>% filter(attempt_number == 1), aes(x=scaled_func_examples, y=correct_120, group=monkey_name, color=monkey_name)) +
...
# Overall accuracy summary by monkey
overall_accuracy <- df %>%
  ...
  summarise(
    ...
    median_distance_error = median(dist_from_true, na.rm = TRUE),
    .groups = 'drop'
  )

cat("OVERALL ACCURACY SUMMARY\n")
cat("========================\n")
for(i in 1:nrow(overall_accuracy)) {
  monkey <- ifelse(overall_accuracy$monkey_name[i] == "BP22", "Monkey 1", "Monkey 2")
  cat(sprintf("%s:\n", monkey))
  cat(sprintf("  Total trials: %d\n", overall_accuracy$n_trials[i]))
  cat(sprintf("  Accuracy (80px): %.1f%%\n", overall_accuracy$mean_accuracy_80[i] * 100))
  cat(sprintf("  Accuracy (120px): %.1f%%\n", overall_accuracy$mean_accuracy_120[i] * 100))
  cat(sprintf("  Mean error: %.1f pixels\n", overall_accuracy$mean_distance_error[i]))
  cat(sprintf("  Median error: %.1f pixels\n\n", overall_accuracy$median_distance_error[i]))
}
...
pattern_accuracy <- df %>%
  ...
  arrange(monkey_name, desc(mean_accuracy_120))

cat("ACCURACY BY PATTERN TYPE\n")
cat("========================\n")
for(monkey in c("BP22", "BP24")) {
  monkey_name <- ifelse(monkey == "BP22", "Monkey 1", "Monkey 2")
  cat(sprintf("\n%s (sorted by 120px accuracy):\n", monkey_name))
  cat("Pattern Type                | Trials | 80px Acc | 120px Acc | Final Acc | Mean Error\n")
  cat("---------------------------|--------|----------|-----------|-----------|----------\n")
  
  monkey_data <- pattern_accuracy %>% filter(monkey_name == monkey)
  for(i in 1:nrow(monkey_data)) {
    pattern <- gsub("_", " ", monkey_data$func.type[i])
    pattern <- str_to_title(pattern)
    cat(sprintf("%-26s | %6d | %7.1f%% | %8.1f%% | %8.1f%% | %7.1f px\n",
                pattern,
...
# Compare first 25% vs last 25% of training for each pattern
early_late_comparison <- df %>%
  filter(attempt_number == 1) %>%
  group_by(monkey_name, func.type) %>%
  filter(n() >= 20) %>%  # Only patterns with sufficient data
  mutate(
    quartile = ntile(func_examples, 4),
    phase = case_when(
      quartile == 1 ~ "Early",
      quartile == 4 ~ "Late",
      TRUE ~ "Middle"
  ...
  group_by(monkey_name, func.type, phase) %>%
  summarise(
    mean_accuracy = mean(correct_120, na.rm = TRUE),
    n_trials = n(),
    .groups = 'drop'
  ) %>%
  pivot_wider(names_from = phase, values_from = c(mean_accuracy, n_trials)) %>%
  mutate(
    improvement = mean_accuracy_Late - mean_accuracy_Early,
    relative_improvement = improvement / (mean_accuracy_Early + 0.01)
  ) %>%
  arrange(monkey_name, desc(improvement))
...
for(monkey in c("BP22", "BP24")) {
  monkey_name <- ifelse(monkey == "BP22", "Monkey 1", "Monkey 2")
  cat(sprintf("\n%s:\n", monkey_name))
  cat("Pattern Type                | Early | Late  | Improvement | Relative\n")
  cat("---------------------------|-------|-------|-------------|----------\n")
  
  monkey_data <- early_late_comparison %>% filter(monkey_name == monkey)
  for(i in 1:nrow(monkey_data)) {
    pattern <- gsub("_", " ", monkey_data$func.type[i])
    pattern <- str_to_title(pattern)
    cat(sprintf("%-26s | %4.1f%% | %4.1f%% | %10.1f%% | %7.1f%%\n",
                pattern,
                monkey_data$mean_accuracy_Early[i] * 100,
                monkey_data$mean_accuracy_Late[i] * 100,
                monkey_data$improvement[i] * 100,
                monkey_data$relative_improvement[i] * 100))
  }
}
...
for(monkey in c("BP22", "BP24")) {
  ...
  positive_improvements <- sum(monkey_data$improvement > 0, na.rm = TRUE)
  total_patterns <- nrow(monkey_data)
  mean_improvement <- mean(monkey_data$improvement, na.rm = TRUE) * 100
  
  cat(sprintf("%s: %d/%d patterns improved, average improvement: %.1f%%\n",
              monkey_name, positive_improvements, total_patterns, mean_improvement))
}
