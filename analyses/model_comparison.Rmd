...
fname <- "../figs/learning_curves/adult_lot_gpsl.png"
...
# Lets make scatters for accuracy
```{r}
...
plot_df_monkey <- df_monkey %>% mutate(agent="Monkeys") %>% select(correct, tpt, subj_id, func.name, agent) #%>% group_by(func.name) #%>% summarize(mean_p_true_monkey = mean(correct, na.rm=T))
...
ggsave("../figs/scatters/kids_vs_monkeys.png", plot = last_plot(), width = 5, height = 4.4, units = "in", bg="white")
```
...
compute_ll <- function(df_model, df_participant, out_path) {
  ...
  # combined participant and model df
  df_participant_model <- add_new_columns_grouped(df_participant, df_model, get_model_data, c("func.name", "n"), "model", 
                                                  c("model_pred_x", "model_std_x", "model_pred_y", "model_std_y", "model_posterior", "model_particle")) %>%
  ...
  ll <- sum(log(df_result$p_data_x), na.rm=TRUE) + sum(log(df_result$p_data_y), na.rm=TRUE)
  #print(-ll)
  print(sum(df_result$LL, na.rm=T))
...
```{r}
sm <- 1e-10 
compute_ll_true <- function(df_participant, out_path) {
  # function to optimize
  fn <- function(pars) {
    o_sd <- pars[1]
    l_p <- pars[2]
    ...
    df_result <- df_optim %>%
              filter(n > 2) %>%
              group_by(subj_id, func.name, r_id, n) %>%
              mutate(p_data_x = sm + (1-sm) * dnorm(pred_x, mean=true_x, sd=o_sd)) %>%
              mutate(p_data_y =  sm + (1-sm) * dnorm(pred_y, mean=true_y, sd=o_sd)) %>%
              mutate(p_data_x =(1-l_p)* p_data_x + l_p * dnorm(pred_x, mean=prev_x, sd=l_sd)) %>%
              mutate(p_data_y =(1-l_p)* p_data_y + l_p * dnorm(pred_y, mean=prev_y, sd=l_sd)) 
    ll <- sum(log(df_result$p_data_x), na.rm=TRUE) + sum(log(df_result$p_data_y), na.rm=TRUE)
    return(-ll)
  ...
  df_participant_model <- df_participant %>%
                          group_by(func.name) %>%
                          mutate(min_x = mean(min_x, na.rm=T), min_y = mean(min_y, na.rm=T),
                                 max_x = mean(max_x, na.rm=T), max_y = mean(max_y, na.rm=T)) %>%
                          ungroup() %>%
...
```{r}
sm <- 1e-10 
compute_ll_prev <- function(df_participant, out_path) {
  # function to optimize
  fn <- function(pars) {
    o_sd <- pars[1]
    df_result <- df_optim %>%
              filter(n > 2) %>%
              group_by(subj_id, func.name, r_id, n) %>%
              mutate(p_data_x = sm + (1-sm) * dnorm(pred_x, mean=prev_x, sd=o_sd)) %>%
              mutate(p_data_y =  sm + (1-sm) * dnorm(pred_y, mean=prev_y, sd=o_sd))
    ll <- sum(log(df_result$p_data_x), na.rm=TRUE) + sum(log(df_result$p_data_y), na.rm=TRUE)
    return(-ll)
  }
  ...
  for(subj in unique(df_participant_model$subj_id)) {
    df_optim <- df_participant_model %>% filter(subj_id==subj)
    op_pars <- optim(c(other_noise), fn,
          method = "L-BFGS-B",
          lower = c(1e-5), upper = c(5))
    pars <- op_pars$par
    subj_param_dict[[subj]] <- pars
  }
...
```{r, message=F}
# Compute participant LL under learning models
#model_paths = c(lin_path, ridge_path)#lot_path, gpnc_path, gpsl_path)
#models = c( "lin", "ridge")#"lot", "gpnc", "gpsl", "lin", "ridge", "true")
...
#     compute_ll(df_model, df_participant, out_file)
#   }
# }


#Compute participant LL control model assuming participants guess around true point
for (j in seq_along(participants)) {
    if (participants[j] == 'adult') {
      df_participant <- df_adult
    } else if (participants[j] == 'kid') {
    ...
    print("true")
    print(participants[j])
    out_file <- paste("model_fits/LLs/", "true", "_", participants[j], ".csv", sep="")
    compute_ll_true(df_participant, out_file)
}
...
#     print("rand")
#     print(participants[j])
#     out_file <- paste("model_fits/LLs/", "prev", "_", participants[j], ".csv", sep="")
#     compute_ll_prev(df_participant, out_file)
# }
...
rand_kid <- read.csv(paste(path, "prev_kid.csv", sep=""))
...
rand_adult <- read.csv(paste(path, "prev_adult.csv", sep=""))
...
rand_monkey <- read.csv(paste(path, "prev_monkey.csv", sep=""))
...
get_AIC <- function(all_fits) {
  ...
  df_aic <- grouped %>%
            ...
            group_by(model) %>%
            mutate(n_params = ifelse(model%in%c("Prev"), n_subjs, n_subjs*3)) %>% #only one param for prev pt model
...
all_fits_adult <- bind_rows(lot_adult %>%mutate(model="LoT"),
                           ...
                           true_adult%>%mutate(model="True"),
                           rand_adult%>%mutate(model="Prev"))
...
all_fits_kid <- bind_rows(lot_kid%>%mutate(model="LoT"),
                         ...
                         true_kid%>%mutate(model="True"),
                         rand_kid%>%mutate(model="Prev"))
...
all_fits_monkey <- bind_rows(lot_monkey%>%mutate(model="LoT"),
                            ...
                            true_monkey%>%mutate(model="True"),
                            rand_monkey%>%mutate(model="Prev"))
...
ages <- top %>% group_by(model) %>% summarize(age=mean(age))
lot_ages <- (top %>% filter(model=="LoT"))$age
lin_ages <- (top %>% filter(model=="Lin."))$age
gpsl_ages <- (top %>% filter(model=="GPSL"))$age
gpnc_ages <- (top %>% filter(model=="GPNC"))$age
ridge_ages <- (top %>% filter(model=="Ridge"))$age

t.test(lin_ages, gpsl_ages)
