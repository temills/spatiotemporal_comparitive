...
```
```{r}
write.csv(tmp, '../mia/polynomial.csv')
```
  ...
  for(i in seq_along(model_name_list)) {
      ...
      model_df <- model_df_list[[i]] %>%
            ...
            mutate(x_pred = ifelse(x_pred < 0, 0, x_pred),
                   ...
                   y_pred = ifelse(y_pred > 0.75, 0.75, y_pred)) %>%
            group_by(func.type, tpt) %>%
            arrange(particle) %>%
...
refactor_posterior <- function(posterior_samples) {
  refactored_post <- posterior_samples %>%
                     #going from .subj.model to .model.subj
                     rename_with(~ gsub("posterior_probs\\.(\\d+)\\.(\\d+)", "posterior_probs.\\2.\\1", .x)) %>%
                     rename_with(~ gsub("log_likelihoods\\.(\\d+)\\.(\\d+)", "log_likelihoods.\\2.\\1", .x)) %>%
                     rename_with(~ gsub("p_lapse\\.(\\d+)\\.(\\d+)", "p_lapse.\\2.\\1", .x)) %>%
                     rename_with(~ gsub("lapse_sd\\.(\\d+)\\.(\\d+)", "lapse_sd.\\2.\\1", .x)) %>%
                     rename_with(~ gsub("motor_sd\\.(\\d+)\\.(\\d+)", "motor_sd.\\2.\\1", .x)) %>%
                     #going from .subj.model.strategy to .strategy.model.subj
                     rename_with(~ gsub("p_lapse_type\\.(\\d+)\\.(\\d+)\\.(\\d+)", "p_lapse_type.\\3.\\2.\\1", .x)) %>%
                     #make .subj part of all these columns into a new column
                     pivot_longer(cols = matches("p_lapse\\.\\d+|lapse_sd\\.\\d+|motor_sd\\.\\d+|posterior_probs\\.\\d+|log_likelihoods\\.\\d+"),
                                  names_to = c(".value", "subj"),
                                  names_pattern = "(p_lapse.\\d+|lapse_sd.\\d+|motor_sd.\\d+|p_lapse_type.\\d+.\\d+|posterior_probs.\\d+|log_likelihoods.\\d+)\\.(\\d+)")          
...
run_inference_mixture <- function(stan_df, n_iter, n_chains, model_name_list) {
  stan_data <- get_stan_data(stan_df, model_name_list)
  model_path <- "stan_models/infer_p_models_mixture.stan"
  fit <- stan(file=model_path, data=stan_data, iter=n_iter, chains=n_chains, cores=n_chains)
  return(fit)
}
...
# Mixture
```{r}
#fit_children_mixture <- readRDS("model_fits/infer_p_models_fit_kids_mixture2.rds")
fit_children_mixture <- readRDS("../fit_children_mixture.rds")
...
```
```{r}
traceplot(fit_children_mixture, pars="p_lapse")
```
# Group posterior
```{r}
...
```{r}
ggplot(data=plot_df) +
  geom_boxplot(y=p_lapse, x=subject_id)
```
