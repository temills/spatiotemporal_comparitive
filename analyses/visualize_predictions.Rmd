...
# grid-ify model probs
```{r}
gaussian_2d <- function(x_min, x_max, y_min, y_max, mu_x, mu_y, sd_x, sd_y) {
  return ((pnorm(x_max - mu_x, 0, sd_x) - pnorm(x_min-mu_x, 0, sd_x)) * (pnorm(y_max - mu_y, 0, sd_y) - pnorm(y_min-mu_y, 0, sd_y)))
...
make_func_vid <- function(funcname) {
  ...
  for (i in (min(df_participant$tpt)+1):max(df_participant$tpt)) {
    ...
    p1 <- ggplot() +
          ...
          # formatting
          scale_color_manual(labels = c("agent1"=agent1, "agent2"=agent2),
                            values=c("agent1"="maroon", "agent2"="green"),
                            breaks = c("agent1", "agent2")) +
    ...
    # cell edges
    x_edges <- seq(x_min - (range_x * 0.25), x_min + (range_x * 1.25), length.out = 150)
    y_edges <- seq(y_min - (range_y * 0.25), y_min + (range_y * 1.25), length.out = 100)
    # cell bounds
    ...
    density_vec <- rowSums(density_arr)
    grid$d <- density_vec
    ...
    if (i<3) {
      ...
      p3 <- p2 +
            # model predictions
            geom_tile(data=grid, aes(x = x_center, y = y_center, alpha=log(d)), fill="green") +
            # participant predictions
...
model_list <- c("LoT", "Nonrecursive LoT", "GP", "Comp. GP", "Ridge", "Linear", "Linear + Prev.", "Transformer (2)", "Transformer (3)", "Transformer (13)")
participant_list <- c("3 yos", "4 yos", "5 yos", "6 yos", "7 yos", "Adults", "Monkeys", "Monkey 1", "Monkey 2")
...
i = 3
    ...
    density_vec <- rowSums(density_arr)
    grid$d <- density_vec
    grid <- grid %>% filter(d > 1e-10)

p2 +
    # model predictions
    geom_point(data=df_model_tpt, aes(x=x_pred, y=y_pred), color="blue") +
...
# why is prob only left and below particles
gaussian_2d <- function(x_min, x_max, y_min, y_max, mu_x, mu_y, sd_x, sd_y) {
  return((pnorm(x_max, mu_x, sd_x) - pnorm(x_min, mu_x, sd_x)) *
         (pnorm(y_max, mu_y, sd_y) - pnorm(y_min, mu_y, sd_y)))
}


funcname = "123"
df_participant <- df_kid_chs %>% filter(age==3)
df_model <- df_lot#lin_prev
df_seq <- subset(df_participant, (df_participant$func.type == funcname))
#dir.create(paste0(out_folder, "/", funcname))
...
# cell edges
x_edges <- seq(x_min - (range_x * 0.25), x_min + (range_x * 1.55), length.out = 100)
y_edges <- seq(y_min - (range_y * 0.25), y_min + (range_y * 1.25), length.out = 100)
# cell bounds
x_mins <- head(x_edges, -1)
x_maxs <- tail(x_edges, -1)
...
df_model_tpt <- df_model_tpt %>%
                mutate(posterior = posterior+1000,
...
ggplot() +
        geom_rect(data=grid, aes(xmin = x_min, xmax = x_max, ymin = y_min, ymax = y_max), linewidth=0.2, color="black", fill="white") +
      #geom_point(data=df_model_tpt, aes(x=x_pred, y=y_pred), color="blue", size=.5, stroke=1) +
      #geom_errorbar(data=df_model_tpt, aes(xmin=x_pred-sd_x, xmax=x_pred+sd_x, y=y_pred), width=0, color="blue") +
      #geom_errorbar(data=df_model_tpt, aes(ymin=y_pred-sd_y, ymax=y_pred+sd_y, x=x_pred), width=0, color="blue") +
      # model predictions
      #geom_vline(xintercept=x_max) +
      geom_rect(data=grid, aes(xmin = x_min, xmax = x_max, ymin = y_min, ymax = y_max, alpha=d), fill="green") #+
      #scale_fill_gradient(low = scales::alpha("green", 0), high = "green") +
      # participant predictions
      #geom_point(data=df_model_tpt, aes(x=x_pred, y=y_pred, color="agent2"), size=0.5, stroke=1, alpha=0.7)
      #geom_point(data=df_tpt, aes(x=x_pred, y=y_pred, color="agent1"), size=1.5, stroke=1, alpha=0.7)
...
df_model_tpt <- data.frame(x_pred = c(1),#,2,3),
                           y_pred = c(1)#,2,3)
                           )
df_model_tpt$sd_x = 0.2
df_model_tpt$sd_y = 0.2
df_model_tpt$posterior = 1/3

# cell edges
x_edges <- seq(-20, 20, length.out = 100)
y_edges <- seq(-20, 20, length.out = 100)
# cell bounds
x_mins <- head(x_edges, -1)
x_maxs <- tail(x_edges, -1)
y_mins <- head(y_edges, -1)
...
grid <- grid %>%
        rowwise() %>%
        mutate(d = (pnorm(x_max, 0, 0.1, lower.tail=FALSE) - pnorm(x_min, 0, 0.1, lower.tail=FALSE)) * (pnorm(y_max, 0, 0.1, lower.tail=FALSE) - pnorm(y_min, 0, 0.1, lower.tail=FALSE)))
...
p3    
print(sort(density_vec, decreasing=T))
```
```{r}

# cell edges
x_edges <- seq(-20, 20, length.out = 100)
y_edges <- seq(-1, 1, length.out = 2)
# cell bounds
x_mins <- head(x_edges, -1)
x_maxs <- tail(x_edges, -1)
y_mins <- head(y_edges, -1)
...
grid <- grid %>%
        rowwise() %>%
        mutate(d= (pnorm(x_max, 0,4) - pnorm(x_min, 0,4)))
        #mutate(d = (pnorm(x_max, 0, 0.1) - pnorm(x_min, 0, 0.1)) * (pnorm(y_max, 0, 0.1) - pnorm(y_min, 0, 0.1)))
...
ggplot() +
        geom_rect(data=grid, aes(xmin = x_min, xmax = x_max, ymin = y_min, ymax = y_max),
                  linewidth=0.2, color="black", fill="white") +
        geom_rect(data=grid, aes(xmin = x_min, xmax = x_max, ymin = y_min, ymax = y_max, alpha=d), fill="green") #+
...
```{r}
ggplot() +
  geom_raster(data=grid, aes(x = x, y = y, fill=d), interpolate = TRUE) +
  scale_fill_gradient(low="white", high="#516797") +
  coord_equal() +
  theme_minimal() +
  labs(fill = "Density") #+
  #geom_point(data = df_model_tpt, aes(x = x_pred, y = y_pred), color = "white", size = 2)
