...
```{r}
df_stimuli_all <- df_stimuli %>%
              group_by(func.name) %>%
              arrange(tpt)
df_adult_all <- df_adult %>%
              group_by(func.name) %>%
              arrange(tpt)
plot_tpt <- function(t) {
    df_adult <- df_adult_all %>%
                mutate(alpha_val = ifelse(tpt>t, 0, 0.75))
    df_stimuli <- df_stimuli_all %>%
                  mutate(alpha_val = ifelse(tpt>t, 0, 0.75))
    p1 <- ggplot() +
      geom_point(data=df_stimuli, aes(x=true_x, y=-true_y, alpha=alpha_val), size=1.2) +
      geom_path(data=df_stimuli%>%filter(tpt<=t), aes(x=true_x, y=-true_y, alpha=alpha_val), color="black", size=0.2) +
      geom_point(data=df_adult, aes(x=pred_x, y=-pred_y, color=tpt, alpha=alpha_val), size=0.6) +
      scale_color_gradientn(colors=c("darkblue", "darkred", "orange", "gold"), name="Timepoint") +
      paper_theme +
...
for (t in (0:20)) {
  ggsave(paste0("../slide_figs/", "stimuli_", t, ".png"), plot_tpt(t), width=8, height=3.5)
}
