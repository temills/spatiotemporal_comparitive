...
# Look at kids data
```{r}
df_kid <- preprocess_df_human(read.csv("../human_experiments/dots_app_kids/data/clean_data/data.csv"))#"data/participants/kids_chs.csv")) 
```
```{r}
tmp <- df_kid %>%
      group_by(subj_id) %>%
      mutate(n_funcs = length(unique(func.name))) %>%
      ungroup() %>%
      group_by(subj_id, func.name) %>%
      mutate(func_max_n=max(n)) %>%
      ungroup()
```

```{r}
table(tmp$func_max_n)

tmp2 <- tmp %>%
        group_by(subj_id, age) %>%
        summarize(n_funcs=mean(n_funcs))

ggplot(data=tmp2) +
  geom_bar(aes(x=n_funcs)) +
...
compute_fit <- function(df) {
  m <- glmer(
            correct ~ 1 +                          # fixed intercept
                      tpt_std +                    # fixed slope 
                      (1 | subj_id) +    # random intercept + slope for subject
