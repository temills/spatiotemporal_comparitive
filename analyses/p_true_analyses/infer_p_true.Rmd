...
```{r}
tmp <- get_subj_df(df_monkey, "monkeys", "BP22")
```
...
# Run inference, compute posterior means
```{r}
#run_inference_group(df_kid_chs, "kids_chs")
for (x in 3:7) {
  df_age <- df_kid_chs %>% filter(age==x)
  run_inference_group(df_age, paste0("kid_chs_", x))
}
...
```{r}
df_kid_chs <- df_kid_chs %>% filter(subject_id != "6QWTWP")

df_mean_kid_chs <- data.frame()
for (x in 3:7) {
  df_mean_kid_chs <- bind_rows(df_mean_kid_chs, get_mean_df(df_kid_chs%>%filter(age==x), paste0("kid_chs_", x)))
}

write.csv(df_mean_kid_chs, "posterior_means/mean_kid_chs.csv")
write.csv(df_mean_adult, "posterior_means/mean_adult.csv")
write.csv(df_mean_monkey, "posterior_means/mean_monkey.csv")
```
# Or load cached posterior means
```{r}
# df_mean_adult <- read.csv("posterior_means/mean_adult.csv")
# df_mean_monkey <- read.csv("posterior_means/mean_monkey.csv")
# df_mean_kid_chs <- read.csv("posterior_means/mean_kid_chs.csv")
...
for (x in 3:7) {
  plot_scatter(df_mean_kid_chs%>%filter(age==x), df_mean_monkey, paste0(x,"yo"),  "Monkeys")
  plot_scatter(df_mean_kid_chs%>%filter(age==3), df_mean_adult, paste0(x,"yo"),  "Adults")
}
```
