...
```{r}
tmp <- df_kid %>%
       group_by(agent, func.name, model) %>%
...
```
```{r}
ggplot(data=df_kid%>%filter(func.name=="alternating_diff_1")) +
  geom_point(aes(x=true_x, y=true_y), color="blue") +
  #geom_point(aes(x=pred_x, y=pred_y), alpha=0.2, color="gold") +
  geom_point(aes(x=model_pred_x, y=model_pred_y), alpha=0.2, color="red") +
  facet_wrap(~func.name+model)
```
```{r}
ggplot(df_all %>% filter(agent=="Adults") %>% filter(model=="Linear")) +
  geom_point(aes(x=true_x, y=true_y), color="blue") +
  geom_point(aes(x=pred_x, y=pred_y), alpha=0.2, color="gold") +
  geom_point(aes(x=model_pred_x, y=model_pred_y), alpha=0.2, color="red") +
...
ggplot(tmp) +
  geom_histogram(aes(x=LL)) +
  facet_wrap(~agent+model)
```
