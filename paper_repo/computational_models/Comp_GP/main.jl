...
@gen function subtree_proposal(prev_trace, idx, biased::Bool)
    ...
    root = prev_trace[][idx]
    # Regenerate
    (subtree_node, subtree_idx, subtree_depth) = @trace(pick_node(root, 1, 0), :choose_subtree_root)
    cps = begin
        if !changepoints
            false
...
function subtree_involution_x(
    ...
    (new_trace, weight, retdiff, discard) = update(trace, model_args, (NoChange(),), constraints)
