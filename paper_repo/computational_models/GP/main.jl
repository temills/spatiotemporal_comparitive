...
# Adapted from:
# Saad FA, Patton BJ, Hoffmann MD, Saurous RA, Mansinghka VK (2023). Sequential monte carlo learning for time series structure discovery in International Conference on Machine Learning. 1025 (PMLR). 
################################

@gen function covariance_prior(cur::Int, node_dist)
...
@gen function model(input::Vector{Vector{Float64}}, node_dist)
    n = length(input[1])
    covariance_fn_x = @trace(covariance_prior(1, node_dist), :tree_x)
    covariance_fn_y = @trace(covariance_prior(1, node_dist), :tree_y)
    noise_x = @trace(gamma(noise_α, noise_β), :noise_x)
    noise_y = @trace(gamma(noise_α, noise_β), :noise_y)
...
@gen function noise_proposal_x(prev_trace)
    return @trace(gamma(noise_α, noise_β), :noise_x)
end
@gen function noise_proposal_y(prev_trace)
    return @trace(gamma(noise_α, noise_β), :noise_y)
end
...
function experiment_smc(
    ...
    if isnothing(kernel_type)
        node_dist = node_dist_leaf
    else
        node_dist = zeros(kernel_type)
        node_dist[kernel_type] = 1.
    end
    ...
    for t=t_start:t_step:length(input[1])-1
        ...
        Threads.@threads for i=1:n_particles
            ...
            for iter=1:n_mcmc
                # MH moves on kernel types
                tr, accepted = metropolis_hastings(tr, Gen.select(:tree_x=>(1, :type)))
                tr, accepted = metropolis_hastings(tr, Gen.select(:tree_y=>(1, :type)))
                # MH move on leaf nodes
