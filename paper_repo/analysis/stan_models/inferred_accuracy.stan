data {
  ...
  vector<lower=0, upper=1>[N] pred_x;
  vector<lower=0, upper=1>[N] pred_y;
  vector<lower=0, upper=1>[N] true_x;
  vector<lower=0, upper=1>[N] true_y;
  vector<lower=0, upper=1>[N] prev_x;
  vector<lower=0, upper=1>[N] prev_y;
...
model {
  ...
  beta_guess_num_func_type ~ normal(0,3); 
  ...
  for (i in 1:N) {
    ...
    vector[3] log_probs;
    log_probs[1] = log(p_true) + normal_lpdf(pred_x[i] | true_x[i], motor_sd) + normal_lpdf(pred_y[i] | true_y[i], motor_sd);
    //otherwise 
    log_probs[2] = log((1-p_true) * p_rand_strategy[1]) + normal_lpdf(pred_x[i] | prev_x[i], prev_sd) + normal_lpdf(pred_y[i] | prev_y[i], prev_sd);
    log_probs[3] = log((1-p_true) * p_rand_strategy[2]) + uniform_lpdf(pred_x[i] | 0, upper_bound_x[i]) + uniform_lpdf(pred_y[i] | 0, upper_bound_y[i]);
