...
## Humans (ages 3–7 and Adults) vs Monkey accuracy
...
lin_df <- preprocess_df_children(read.csv(paste0(data_dir, "participants/kids.csv"))) %>%
          ...
          mutate(delta_x = prev_x - lag(prev_x),
                 ...
                 lin_correct = as.numeric(lin_error <= acceptance_dist)) %>%
          filter(tpt>2) %>%
          summarize(lin_accuracy = mean(lin_correct))

func_accuracy_df <- df_all_modeled %>%
                    #df_all %>%
                    ...
                    group_by(agent, func.name) %>%
                    summarize(accuracy=mean(accuracy), .groups="drop") %>%
                    merge(lin_df, by="func.name")

corr_df <- func_accuracy_df %>%
           group_by(agent) %>%
           summarize(r=cor.test(accuracy, lin_accuracy)[["estimate"]],
                     p=cor.test(accuracy, lin_accuracy)[["p.value"]])

ggplot(func_accuracy_df,
       aes(x=lin_accuracy, y=accuracy)) +
