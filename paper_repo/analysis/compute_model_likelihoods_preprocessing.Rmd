...
clamp <- function(x, lower, upper) {
  min(max(x, lower, na.rm=T), upper, na.rm=T)
}
...
get_df_participant_model <- function(df_participant, df_model, bounds_df) {
  df_participant_model <- add_new_columns_grouped(df_participant, df_model, get_model_data, c("func.name", "tpt"), "model",
                          ...
                          filter(!is.na(model_pred_x)) %>%
                          rowwise() %>%
                          # clamp predictions within bounds
                          mutate(model_pred_x = clamp(model_pred_x, min_bound_x, max_bound_x),
                                 model_pred_y = clamp(model_pred_y, min_bound_y, max_bound_y),
                                 pred_x = clamp(pred_x, min_bound_x, max_bound_x),
                                 pred_y = clamp(pred_y, min_bound_y, max_bound_y)) %>%
                          ...
                          mutate(range_x = (max_bound_x-min_bound_x),
                                 ...
                                 model_pred_x = (model_pred_x-min_bound_x)/range_xy,
                                 model_pred_y = (model_pred_y-min_bound_y)/range_xy,
...
write_preprocessed_data <- function() {
  models = c("transformer_1", "transformer_4", "transformer_5", "transformer_6", "transformer_8", "transformer_10", "transformer_2", "transformer_3", "transformer_13", "lot", "lot_nonrecursive", "linear_or_prev", "gp", "comp_gp", "ridge", "linear") 
  participants = c('kids', 'adults', "monkeys")
  for (i in seq_along(models)) { 
    model_name <- models[i]
    print(model_name)
    model_path <- paste0(data_dir, "models/", model_name, ".csv")
    if (model_name %in% c("linear_or_prev", "linear_or_prev_1")) {
    ...
    for (j in seq_along(participants)) {
      ...
      df_participant_model <- get_df_participant_model(df_participant, df_model, bounds_df_test)
...
bounds_df_test <- bind_rows(df_adult %>% distinct(true_x, true_y, tpt, func.name, min_x, min_y, max_x, max_y) %>% mutate(agent="Adults"),
                            df_kid %>% distinct(true_x, true_y, tpt, func.name, min_x, min_y, max_x, max_y) %>% mutate(agent="Kids"),
                            df_monkey %>% distinct(true_x, true_y, tpt, func.name, min_x, min_y, max_x, max_y) %>% mutate(agent="Monkeys")) %>%
                  group_by(func.name) %>%
                  summarize(min_bound_x = max(min_x, na.rm=T),
                            min_bound_y = max(min_y, na.rm=T),
                            max_bound_x = min(max_x, na.rm=T),
                            max_bound_y = min(max_y, na.rm=T))
...
write_preprocessed_data_monkey_training <- function() {
  ...
  bounds_df_train <- df_participant %>%
                     distinct(true_x, true_y, tpt, func.name, min_x, min_y, max_x, max_y) %>%
                     group_by(func.name) %>%
                     summarize(min_bound_x = max(min_x, na.rm=T),
                               min_bound_y = max(min_y, na.rm=T),
                               max_bound_x = min(max_x, na.rm=T),
  ...
  for (i in c(1,2,3,4,5,6,8,10,13)) {
    ...
    df_participant_model <- get_df_participant_model(df_participant, df_model, bounds_df_train) %>%
                            mutate(tpt=tpt+1)
