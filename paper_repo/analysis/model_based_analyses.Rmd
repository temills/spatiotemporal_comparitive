...
read_model_fit_data <- function(participant) {
  path <- "model_fits/"
  models = c("lot", "gp", "comp_gp", "ridge", "linear", "linear_or_prev", "lot_nonrecursive", "transformer_2", "transformer_3", "transformer_13") 
  df <- data.frame()
...
df_all <- bind_rows(df_adult, df_kid, df_monkey) %>%
          mutate(model = ifelse(model=="comp_gp", "Comp. GP", model),
                  ...
                  model = ifelse(model=="ridge", "Parametric", model),
...
df_all_nonrecursive$model = factor(df_all_nonrecursive$model,
                                   levels = c("Linear",  "Linear + Prev.",  "Parametric",  "GP",  "Comp. GP", "LoT", "Nonrecursive LoT"))

df_all_transformer <- df_all %>%
                      filter(model != "Nonrecursive LoT") %>% 
                      filter(!(func.name %in% c("123", "decreasing_y", "radial_increasing")))
df_all_transformer$model = factor(df_all_transformer$model, levels = c("Linear", "Linear + Prev.",  "Parametric",  "GP",  "Comp. GP", "LoT", "Transformer (2)", "Transformer (3)", "Transformer (13)"))

df_all <- df_all %>% 
          filter(!(model %in% c("Transformer (2)", "Transformer (3)", "Transformer (13)", "Nonrecursive LoT")))
df_all$model = factor(df_all$model, levels = c("Linear",  "Linear + Prev.",  "Parametric",  "GP",  "Comp. GP", "LoT"))
...
mean_LL_df <- df_all %>% 
              ...
              summarize(mean_LL = mean(mean_LL)) %>%
...
resample_by_id <- function(data) {
  ...
  # for each id in boot_ids, add rows with that id to the df
  return(tibble(data_id = boot_ids, boot = seq_along(boot_ids)) %>%  
         left_join(data, by = "data_id", relationship="many-to-many"))
}
...
monkey_table_df <- table_df %>%
       ...
       arrange(factor(model, levels = c("LoT", "Comp. GP", "GP", "Parametric", "Linear + Prev.", "Linear")))
...
top_model_df <- df_all %>%
                ...
                summarize(func_LL = sum(LL, na.rm = TRUE),
                          ...
                          .groups="drop") %>%
                group_by(agent, model, subj_id, func.name) %>%
                summarize(func_LL = mean(func_LL, na.rm = TRUE),
                          p_lapse=mean(p_lapse),
                          sd_motor=mean(sd_motor),
                          sd_prev=mean(sd_prev),
                          p_prev=mean(p_prev),
...
make_barplot <- function(df_all, colormap) {
  ...
  df_top <- df_all %>%
    ...
    summarize(func_LL = sum(LL, na.rm = TRUE), .groups="drop") %>%
    group_by(agent, model, subj_id, func.name) %>%
    summarize(func_LL = mean(func_LL, na.rm=TRUE), .groups="drop") %>%
    group_by(agent, model, subj_id) %>%
...
# Model comparison with transformers
```{r}
...
```{r}
n_samples <- 10000
n_rows <- nrow(ll_monkey_test)
all_samples_x <- rnorm(n_rows * n_samples)
all_samples_y <- rnorm(n_rows * n_samples)
monkey_model_test <- ll_monkey_test %>%
                 mutate(mode="test") %>%
                 mutate(correct_distance=correct_distance/range_xy) %>%
                 mutate(model_error_x = (model_pred_x - true_x),
                ...
                mutate(model_correct = {
                            ...
                            mask <- (samples_x > 0) & (samples_x < max_x) & (samples_y > 0) & (samples_y < max_y)
                 ...
                 group_by(context_len, func.name, subj_id) %>%
                 summarize(monkey_acc=mean(correct), model_acc=mean(model_correct, na.rm=T)) %>%
                 group_by(context_len, func.name) %>%
                 summarize(monkey_acc=mean(monkey_acc), model_acc=mean(model_acc, na.rm=T))

df_test_acc_by_func <- monkey_model_test %>%
          filter(!is.na(context_len) & context_len %in% c(1,2,3,13)) 

df_test_patterns <- read.csv(paste0(data_dir, "participants/kids.csv")) %>%
                    rename(func.name=seq_id) %>%
                    ...
                    distinct(func.name, tpt, true_x, true_y) %>%
                    crossing(context_len = c(1,2,3,13)) %>%
                    left_join(df_test_acc_by_func, by = c("func.name", "context_len")) %>%
                    filter(!is.na(model_acc) & !is.na(monkey_acc)) %>%
                    arrange(func.name, tpt)

scale_fn <- 0.12
aspect_ratio <- 1.0

correlations_test <- df_test_acc_by_func %>%
  group_by(context_len) %>%
  summarize(r = cor(model_acc, monkey_acc, use = "complete.obs"),
            r2 = r^2,
            .groups = 'drop') %>%
  mutate(mode = "Test")
...
test_plot <- ggplot() +
  ...
  geom_path(data = df_test_patterns,
            aes(x = model_acc + true_x * scale_fn,
                y = monkey_acc + true_y * scale_fn * aspect_ratio,
                group = func.name, color = func.name),
            alpha = 0.7, size = 0.4) +

  geom_point(data = df_test_patterns,
             aes(x = model_acc + true_x * scale_fn,
                 y = monkey_acc + true_y * scale_fn * aspect_ratio,
                 group = func.name, color = func.name),
             alpha = 0.8, size = 0.3) +
  ...
  geom_text(data = correlations_test,
            aes(x = 0.12, y = 0.9,
                label = paste("italic(r) ==", sprintf("%.2f", r))),
            size = 4, family = "Georgia", parse = TRUE) +

  labs(x = "Transformer accuracy (test)",
       y = "Monkey accuracy (test)") +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 16),
    ...
    facet_wrap(~context_len, nrow = 2,
           labeller = as_labeller(function(x) paste("Context Length", x)))
print(test_plot)

ggsave(paste0(fig_dir, "monkey_transformer_corr.png"), width=6,height=5,dpi=1000)
```

## Train accuracy correlation
```{r}
n_samples <- 1000
n_rows <- nrow(ll_monkey_train)
all_samples_x <- rnorm(n_rows * n_samples)
all_samples_y <- rnorm(n_rows * n_samples)

monkey_model_train <- ll_monkey_train %>%
                 mutate(mode="train") %>%
                 mutate(correct_distance=80/range_xy) %>%
                 mutate(model_error_x = (model_pred_x - true_x),
                        model_error_y = (model_pred_y - true_y),
                        model_error = sqrt(model_error_x^2 + model_error_y^2),
                        err_x = (pred_x - true_x),
                        err_y = (pred_y - true_y),
                        abs_err = sqrt(err_x^2 + err_y^2),
                        correct = abs_err <= correct_distance) %>%
                rowwise() %>%
                mutate(model_correct = {
                            start_idx <- (row_number() - 1) * n_samples + 1
                            end_idx <- row_number() * n_samples
                 ...
                 group_by(context_len, func.type, subj_id) %>%
                 summarize(monkey_acc=mean(correct), model_acc=mean(model_correct, na.rm=T)) %>%
                 group_by(context_len, func.type) %>%
                 summarize(monkey_acc=mean(monkey_acc), model_acc=mean(model_acc, na.rm=T))

df_train_acc_by_func <- monkey_model_train %>%
          filter(!is.na(context_len) & context_len %in% c(1,2,3,13)) 

correlations_train <- df_train_acc_by_func %>%
  group_by(context_len) %>%
  summarize(r = cor(model_acc, monkey_acc, use = "complete.obs"),
            r2 = r^2,
            .groups = 'drop') 
