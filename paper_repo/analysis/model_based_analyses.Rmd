...
read_model_fit_data <- function(participant) {
  path <- "model_fits/"
  models = c("lot", "gp", "comp_gp", "ridge", "linear_1", "linear_or_prev_1", "lot_nonrecursive", "transformer_2", "transformer_3", "transformer_13") 
  df <- data.frame()
...
df_all <- bind_rows(df_adult, df_kid, df_monkey) %>%
          mutate(model = ifelse(model=="comp_gp", "Comp. GP", model),
                  ...
                  model = ifelse(model=="linear_1", "Linear", model),
                  model = ifelse(model=="ridge", "Parametric", model),
                  model = ifelse(model=="lot", "LoT", model),
                  model = ifelse(model=="linear_or_prev_1", "Linear + Prev.", model),
...
resample_by_id <- function(data) {
  ...
  return(tibble(data_id = boot_ids, boot = seq_along(boot_ids)) %>%  
         left_join(data, by = "data_id"))
}

set.seed(123)
n_boot <- 1000

boot_df <- df_all_transformer %>% 
           compute_top_model() %>%
...
mean_param_df <- df_all_transformer %>%
...
write.csv(stimuli, "../analyses/data/expt_stimuli.csv", row.names=F)
```
...
ggsave(plot=p, paste0(fig_dir, "model_combined_3.png"), bg="white", width=15, height=7)
```
