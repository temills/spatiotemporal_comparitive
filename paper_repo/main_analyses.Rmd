...
ggsave(plot=p, paste0(fig_dir, "stimuli.png"), width=8, height=3, dpi=1000)
```
...
df_monkey <- preprocess_df_monkey(read.csv(paste0(data_dir,  "participants/monkeys.csv"))) %>% filter(tpt>2) %>% mutate(agent="Monkeys")
...
df_kid_modeled <- read.csv(paste0(preprocessed_data_dir, 'inferred_accuracy/mean_kids.csv')) %>%
                  mutate(agent="Kids") %>%
                  rename(subj_id = r_id)
df_monkey_modeled <- read.csv(paste0(preprocessed_data_dir, 'inferred_accuracy/mean_monkeys.csv')) %>%
